...
class RandomMDP(DiscreteMDP):
    def __init__(
        ...
        nbStates,
        nbActions,
        maxProportionSupportTransition=0.5,
        maxProportionSupportReward=0.1,
        maxProportionSupportStart=0.2,
        minNonZeroProbability=0.2,
        minNonZeroReward=0.3,
        rewardStd=0.5,
        ergodic=0.00,
        seed=None,
        name="RandomMDP",
        ...
        for s in self.states:
            ...
            for a in self.actions:
                ...
                my_mean = self.sparserand(
                    p=maxProportionSupportReward, min=minNonZeroReward, max=0.99
                )
                if False and (rewardStd > 0 and my_mean > 0 and my_mean < 1):
                    ma, mb = (0 - my_mean) / rewardStd, (1 - my_mean) / rewardStd
                    self.rewards[s][a] = stat.truncnorm(
                        ma, mb, loc=my_mean, scale=rewardStd
                    )
                else:
                    self.rewards[s][a] = Dirac(my_mean)
                transitionssa = np.zeros((self.nS))
                for s2 in self.states:
                    transitionssa[s2] = max(
                        self.sparserand(
                            p=maxProportionSupportTransition, min=minNonZeroProbability
                        ),
                        ergodic,
                    )
                ...
                if mass > 0:
                    ...
                    transitionssa = self.reshapeDistribution(
                        transitionssa, minNonZeroProbability
                    )
                else:
                    transitionssa[self.np_random.randint(self.nS)] = 1.0
                li = self.P[s][a]
                [
                    li.append((transitionssa[s], s, False))
                    for s in self.states
                    if transitionssa[s] > 0
                ]
                self.transitions[s][a] = {ss: transitionssa[ss] for ss in self.states}

            self.startdistribution[s] = self.sparserand(
                p=maxProportionSupportStart, min=minNonZeroProbability
            )
        mass = sum(self.startdistribution)
        if mass > 0:
            self.startdistribution = self.startdistribution / sum(
                self.startdistribution
            )
            self.startdistribution = self.reshapeDistribution(
                self.startdistribution, minNonZeroProbability
            )
        else:
            self.startdistribution[self.np_random.randint(self.nS)] = 1.0

        checkRewards = sum(
            [
                sum([self.rewards[s][a].mean() for a in self.actions])
                for s in self.states
            ]
        )
        if checkRewards == 0:
            ...
            my_mean = min(
                minNonZeroReward + self.np_random.rand() * (1.0 - minNonZeroReward),
                0.99,
            )
    ...
    def sparserand(self, p=0.5, min=0.0, max=1.0):
        ...
        while mass < 0.99999999:
            ...
            if mdistribution[i] < p:
                ...
                if newp == p:
                    ...
                    mdistribution[i] = p
            if mdistribution[i] >= p:
                newp = min(1.0, mdistribution[i] + 1.0 - mass)
