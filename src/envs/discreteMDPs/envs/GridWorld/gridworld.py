...
def randomMap(sizeX, sizeY, density, lengthofwalks, np_random=np.random):
    ...
    for i in range((int)(density * sizeX * sizeY)):
        p = np.exp(
            -np.log(2) / lengthofwalks
        )  # probability p to continue building the current wall, 1-p  to start a new one.
...
def fourRoomMap(X, Y):
    ...
    for y in range(Y):
        ...
        maze[X - 1][y] = 0.0
        maze[X2][y] = 0.0
        maze[X2][(int)(Y2 / 2)] = 1.0
        maze[X2][(int)(3 * Y2 / 2)] = 1.0
        maze[(int)(X2 / 2)][Y2] = 1.0
        maze[(int)(3 * X2 / 2)][Y2] = 1.0
    return maze
...
class GridWorldWithWall(DiscreteMDP):
    ...
    def __init__(
        ...
        density=0.2,
        lengthofwalks=5,
        initialSingleStateDistribution=False,
        start=None,
        goal=None,
        seed=None,
        name="GridWorldWithWall",
    ):
        """
        ...
        slip = min(slippery, 1.0 / 3.0)

        self.massmap = [
            [slip, 1.0 - 3 * slip, slip, 0.0, slip],  # up : left up right  down stay
            [slip, 0.0, slip, 1.0 - 3 * slip, slip],  # down : left up down right stay
            [1.0 - 3 * slip, slip, 0.0, slip, slip],  # left : left up right down stay
            [0.0, slip, 1.0 - 3 * slip, slip, slip],
        ]  # right : left up right down stay
        ...
        if map_name == "2-room":
            ...
            self.maze = randomMap(
                sizeX, sizeY, density, lengthofwalks, np_random=self.np_random
            )
        ...
        super(GridWorldWithWall, self).__init__(
            self.nS,
            self.nA,
            P,
            R,
            isd,
            nameActions=self.nameActions,
            seed=None,
            name=name,
        )
    ...
    def makeGoalStates(self, nb):
        ...
        for g in range(nb):
            ...
            while self.maze[s[0]][s[1]] == 0:
                s = [
                    self.np_random.randint(self.sizeX),
                    self.np_random.randint(self.sizeY),
                ]
            goalstates.append(self.to_s(s))
            self.maze[s[0]][s[1]] = 2.0
        return goalstates
    ...
    def makeInitialDistribution(self, maze):
        isd = np.array(maze == 1.0).astype("float64").ravel()
        isd /= isd.sum()
        return isd
    ...
    def makeRewards(self):
        R = {s: {a: Dirac(0.0) for a in range(self.nA)} for s in range(self.nS)}
        ...
        for s in range(self.nS):
            ...
            if self.maze[x][y] == 2.0:
                for a in range(self.nA):
                    mymean = 0.99
                    if self.rewardStd > 0:
                        ma, mb = (0 - mymean) / self.rewardStd, (
                            1 - mymean
                        ) / self.rewardStd
                        R[s][a] = stat.truncnorm(
                            ma, mb, loc=mymean, scale=self.rewardStd
                        )
                    else:
                        R[s][a] = Dirac(mymean)
        return R
...
class GridWorld(DiscreteMDP):
    ...
    def __init__(
        ...
        slip = min(slippery, 1.0 / 3.0)
        self.massmap = [
            [slip, 1.0 - 3 * slip, slip, 0.0, slip],  # up : up down left right stay
            [slip, 0.0, slip, 1.0 - 3 * slip, slip],  # down
            [1.0 - 3 * slip, slip, 0.0, slip, slip],  # left
            [0.0, slip, 1.0 - 3 * slip, slip, slip],
        ]  # right
    ...
    def makeInitialSingleStateDistribution(self, maze, xy=None):
        ...
        isd = np.zeros(self.nS)
        isd[self.revmapping[self.to_s(xy)]] = 1.0
        return isd
        ...
        for s in range(self.nS):
            ...
            if self.maze[x][y] == 2.0:
                ...
                for a in range(self.nA):
                    li = P[s][a]
                    li.append(
                        (self.massmap[a][0], self.revmapping[self.to_s(ls)], False)
                    )
                    li.append(
                        (self.massmap[a][1], self.revmapping[self.to_s(us)], False)
                    )
                    li.append(
                        (self.massmap[a][2], self.revmapping[self.to_s(rs)], False)
                    )
                    li.append(
                        (self.massmap[a][3], self.revmapping[self.to_s(ds)], False)
                    )
                    li.append(
                        (self.massmap[a][4], self.revmapping[self.to_s(ss)], False)
                    )
