...
class Qlearning:
    ...
    def update(self, state, action, reward, observation):
        self.Q[state, action] = self.Q[state, action] + self.alpha[state, action] * (
            reward + self.gamma * max(self.Q[observation]) - self.Q[state, action]
        )

        self.alpha[state, action] = 1.0 / (1.0 / self.alpha[state, action] + 1.0)

        (u, arg) = allmax(self.Q[state])
        self.policy[state] = [1.0 / len(arg) if x in arg else 0 for x in range(self.nA)]
...
class AdaQlearning:
    def __init__(self, nS, nA):
        ...
        self.alpha = np.zeros((self.nS, self.nA))
        self.gamma = 0.99  # Fairly close to 1
        self.optValue = 1.0 / (1 - self.gamma)
        self.factor = 1.01
    ...
    def update(self, state, action, reward, observation):
        try:
            ...
            self.gamma = (self.optValue - 1.0) / self.optValue
