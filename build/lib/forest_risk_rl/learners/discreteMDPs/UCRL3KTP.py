...
class UCRL3KTP(Agent):
    def __init__(self, nS, nA, env, delta):
        Agent.__init__(self, nS, nA, name="UCRL3-KTP")
        self.nS = nS
        self.nA = nA
        self.t = 1
        self.delta = delta
    ...
    def EVI(self, r_estimate, p_estimate, epsilon=0.01, max_iter=1000):
        ...
        while True:
            ...
            for s in range(self.nS):
                ...
                for a in range(self.nA):
                    ...
                    temp[a] = self.r_upper[s, a] + sum(
                        [u0[ns] * self.p[s, a][ns] for ns in range(self.nS)]
                    )
            ...
            if (max(diff) - min(diff)) < epsilon:
                ...
                self.u = u1 - min(u1)
                print(
                    "[UCRL3-KTP] No convergence of EVI at time ",
                    self.t,
                    " before ",
                    max_iter,
                    " iterations.",
    ...
    def new_episode(self):
        ...
        self.compute_ranges()
        self.EVI(self.r_meanestimate, self.p, epsilon=1.0 / max(1, self.t))
    ...
    def EVI(self, r_estimate, p_estimate, epsilon=0.01, max_iter=1000, nup_steps=5):
        ...
        while True:
            ...
            if (max(diff) - min(diff)) < epsilon:
                ...
                self.u = u1 - min(u1)
                print(
                    "[UCRL3-KTP] No convergence in the EVI at time ",
                    self.t,
                    " before ",
                    max_iter,
                    " iterations.",
