...
class UCRL3(Agent):
    def __init__(self, nS, nA, delta, K=-1):
        ...
        self.deltaSA = delta / (
            nS * nA * (3 + 3 * K)
        )  # 3 for rewards (1 hoeffding, 2 empBernstein), nS for peeling, nS for Berend up, nS for Berend down
        self.observations = [[], [], []]
        ...
        self.r_m2 = np.zeros(
            (self.nS, self.nA)
        )  # For Welford's algorithm to sequentially update the variance.
    ...
    def confbound_HoeffdingLaplace(self, r, n, delta):
        if n == 0:
            return np.infty
        return np.sqrt((1.0 + 1.0 / n) * np.log(np.sqrt(n + 1) / delta) / (2.0 * n))

    def confbound_EmpBersnteinPeeling(self, r, empvar, n, delta):
        if n == 0:
            return np.infty
        elln = self.elln(n, delta)
        return np.sqrt(2.0 * empvar * elln) + 7.0 * elln / 3.0

    def confbound_BernoulliBernsteinPeeling(self, q, n, delta):  # q- pest <=
        if n == 0:
            return np.infty
        elln = self.elln(n, delta)
        return np.sqrt(2.0 * q * (1.0 - q) * elln) + elln / 3.0
    ...
    def confbound_BernoulliSubGaussianLaplace_bar(self, q, n, delta):  # q- pest <=
        ...
        if q < 0.5:
            if q == 0:
                ...
                gb = (1.0 / 2.0 - q) / np.log(1.0 / q - 1)
        else:
            gb = q * (1.0 - q)

        return np.sqrt(
            2.0 * gb * (1.0 + 1.0 / n) * np.log(2.0 * np.sqrt(n + 1) / delta) / n
        )
    ...
    def confbound_BernoullisubGaussianLaplace(self, q, n, delta):  # pest - q <=
        ...
        if (q > 0) and (q < 1):
            if q == 0.5:
                ...
                g = (1.0 / 2.0 - q) / np.log(1.0 / q - 1)
        return np.sqrt(
            2.0 * g * (1.0 + 1.0 / n) * np.log(2.0 * np.sqrt(n + 1) / delta) / n
        )
    ...
    def q_upper(self, pest, n, delta):
        q = search_up(
            lambda x: x - pest
            <= min(
                self.confbound_BernoulliBernsteinPeeling(x, n, delta),
                self.confbound_BernoulliSubGaussianLaplace_bar(x, n, delta),
            ),
            1.0,
            pest,
            epsilon=0.0001,
    ...
    def q_lower(self, pest, n, delta):
        q = search_down(
            ...
            <= min(
                self.confbound_BernoulliBernsteinPeeling(x, n, delta),
                self.confbound_BernoullisubGaussianLaplace(x, n, delta),
            ),
            pest,
            0.0,
            epsilon=0.0001,
    ...
    def m_upper(self, rest, vest, n, delta):
        return min(
            1,
            rest + self.confbound_EmpBersnteinPeeling(1.0, vest, n, delta),
            rest + self.confbound_HoeffdingLaplace(1.0, n, delta),
        )
    ...
    def compute_ranges(self):
        ...
        for s in range(self.nS):
            for a in range(self.nA):
                n = self.Nk[s, a]
                self.r_upper[s, a] = self.m_upper(
                    self.r_meanestimate[s, a], self.r_varestimate[s, a], n, delta
                )
                self.p_upper[s, a]["out"] = self.q_upper(
                    0.0, n, delta
                )  # Outside support
                for next_s in self.supports[s, a]:  # self.p_estimate[s, a].keys():
                    ...
                    if (p >= 1.0) and (n >= 1):
                        ...
                        self.p_lower[s, a][next_s] = max(
                            (0.5) ** (1.0 / n), self.p_lower[s, a][next_s]
                        )
    ...
    def computeSupport(self, s, a, u, sorted_indices, kappa=np.infty):
        ...
        p_in, idx_in = self.aux_compute_Fplus_in(
            s, a, sorted_indices, p_in, idx_in, support
        )
        p_out, idx_out = self.aux_compute_Fplus_out(
            s, a, sorted_indices, p_out, idx_out, support
        )
        ...
        while temp2 > min(kappa, temp) and ll < self.nS - 1:
            while (
                sorted_indices[self.nS - 1 - ll] in support and ll < self.nS - 1
            ):  # To add something that is actually not in the empirical support
                ll += 1
    ...
    def EVI(self, r_estimate, p_estimate, epsilon=0.01, max_iter=1000):
        ...
        while True:
            ...
            kappa0 = 10 * (max(u0) - min(u0)) / (self.Nkmax ** (2.0 / 3.0))
            for s in range(self.nS):
                temp = np.zeros(self.nA)
                for a in range(self.nA):
                    support = self.computeSupport(
                        s, a, u0, sorted_indices, kappa0 * len(self.supports[s, a])
                    )
                    # print("Support of ", s,a," : ", self.supports[s, a], ", ", support)
                    max_p = self.max_proba(
                        sorted_indices, s, a, support
                    )  # Allowed to sum  to <=1
                    # print("Max_p of ",s,a, " : ", max_p)
                    temp[a] = self.r_upper[s, a] + sum(
                        [u0[ns] * max_p[ns] for ns in max_p.keys()]
                    )
            ...
            if (max(diff) - min(diff)) < epsilon:
                ...
                self.u = u1 - min(u1)
                print(
                    "[UCRL3] No convergence of EVI at time ",
                    self.t,
                    " before ",
                    max_iter,
                    " iterations.",
    ...
    def new_episode(self):
        ...
        self.compute_ranges()
        self.EVI(self.r_meanestimate, self.p_estimate, epsilon=1.0 / max(1, self.t))
    ...
    def update(self, state, action, reward, observation):
        self.sumratios = self.sumratios + 1.0 / max([1, self.Nk[state, action]])
        ...
        for next_s in self.p_estimate[state, action].keys():
            self.p_estimate[state, action][next_s] = (
                self.p_estimate[state, action][next_s] * (n - 1.0) / n
            )
        if observation in self.p_estimate[state, action].keys():
            self.p_estimate[state, action][observation] = (
                self.p_estimate[state, action][observation] + 1.0 / n
            )
        else:
            self.p_estimate[state, action][observation] = 1.0 / n
...
class UCRL3_lazy(UCRL3):
    ...
    def EVI(self, r_estimate, p_estimate, epsilon=0.01, max_iter=1000, nup_steps=5):
        ...
        while True:
            ...
            if (
                itera < nup_steps
            ):  # Force checking criterion at all steps before nup_steps
                nup = 1
            for _ in range(nup):
                for s in range(self.nS):
                    temp = np.zeros(self.nA)
                    for a in range(self.nA):
                        temp[a] = self.r_upper[s, a] + sum(
                            [u0[ns] * max_p[s, a][ns] for ns in max_p[s, a].keys()]
                        )
            ...
            if (max(diff) - min(diff)) < epsilon:
                ...
                self.u = u1 - min(u1)
                print(
                    "No convergence in the EVI at time ",
                    self.t,
                    " before ",
                    max_iter,
                    " iterations.",
